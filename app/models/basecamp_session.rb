class BasecampSession < ApplicationRecord
  ...
  def fetch_request_token_url
    client.auth_code.authorize_url(type: 'web_server', redirect_uri: Rails.application.routes.url_helpers.basecamp_authorization_callback_url(protocol: 'https'))
  end
  
  def fetch_access_token!(verifier)
    oauth_access_token = client.auth_code.get_token(verifier, type: 'web_server', redirect_uri: Rails.application.routes.url_helpers.basecamp_authorization_callback_url(protocol: 'https'))
